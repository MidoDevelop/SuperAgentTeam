...
4. 在 prompt 中注入以下必须信息：
   a. 所属团队名: sat-iteration
   b. 任务名
   c. 输出文件路径（如有）
   d. 完成后用 TaskUpdate 标记任务完成
   e. 进度汇报规则（见下方）
5. 使用 Agent tool 启动，指定 team_name: "sat-iteration" 和 name
6. 同一步骤中所有独立成员必须在同一条消息中并行启动
7. Lead 执行监管循环（见下方）
8. 所有成员完成后，向每个成员发送 shutdown_request（SendMessage type: "shutdown_request"）
```

### 进度汇报规则（必须注入到每个团队成员的 prompt 中）

在每个团队成员的 prompt 末尾追加以下指令：

```
## 进度汇报要求

你是 SAT 迭代团队的成员。在执行任务过程中，你必须遵守以下汇报规则：

1. **每 5 分钟主动汇报一次进度**：使用 SendMessage 向 Lead 发送当前进度。
   格式：
   - recipient: Lead 的 name（团队创建者）
   - type: "message"
   - summary: "进度汇报: {任务名}"
   - content 包含：
     - 当前正在做什么
...
2. **遇到阻塞时立即汇报**：不要等到下一个 5 分钟周期，遇到无法解决的问题立即通知 Lead。

3. **完成时汇报**：任务完成后，先用 TaskUpdate 标记完成，再向 Lead 发送完成消息。
```

### Lead 监管职责

你（编排器/Lead）在团队成员工作期间的职责：

1. **接收进度汇报**: 成员每 5 分钟发送的进度消息会自动推送给你
2. **阻塞响应**: 收到阻塞汇报时，评估问题并决定：
   - 向成员提供指导（SendMessage）
   - 向用户报告并请求决策
   - 调整任务分配
3. **超时检测**: 如果某个成员超过 10 分钟未汇报，主动发送消息询问状态
4. **进度同步**: 在关键节点（如所有成员完成、进入 HITL 检查点前），向用户简要汇总团队工作进展
