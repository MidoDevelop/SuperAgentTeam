...
**目标**: 先扫描各端代码库和文档，建立技术上下文，为后续需求分析提供可靠依据。
...
- 注入: **需求文档全文**（作为扫描方向参考） + 该平台的项目路径
- 工作: 先读 CLAUDE.md 理解项目架构，再用 Glob/Grep/Read 扫描代码库，找到相关文件、API、数据模型
- 输出写入: `{output_dir}/阶段1-信息采集/{platform}-代码扫描.md`
...
- 输出写入: `{output_dir}/阶段1-信息采集/文档知识.md`
...
- 注入: **需求文档全文**
- 工作: 搜索技术方案、最佳实践、框架文档
- 输出写入: `{output_dir}/阶段1-信息采集/技术调研.md`
...
- 注入: 需求文档全文 + 所有代码扫描结果 + 文档知识 + 技术调研
- 输出写入: `{output_dir}/阶段1-信息采集/信息整合.md`
- 输出必须包含: 各端项目架构概要、相关代码清单、现有 API 清单、数据模型现状、技术约束、外部参考
...
**目标**: 基于信息采集的代码上下文，从用户、技术、风险三个视角分析需求，形成逐条需求分析文档，标注代码定位、风险项和待确认事项，由用户确认后进入设计。

### 3.1 创建任务 & 并行启动 3 个分析成员

**创建任务**（使用 TaskCreate，一次创建 3 个）：
1. `用户视角分析` — 从用户体验角度分析需求，补充用户故事和验收标准
2. `技术可行性评估` — 基于代码扫描结果，逐条评估每个需求在各端的可行性和改动范围
3. `风险分析` — 基于代码上下文，识别每个需求的技术风险和集成风险
...
- 需求文档全文
- **信息整合结果**（阶段一产出，包含各端代码扫描、项目架构、技术约束等）
- 涉及平台列表
- 各平台项目路径
- 设计原则
- 所属团队名: `sat-iteration`
...
- 已有信息整合结果中的代码扫描作为基础，可按需进一步扫描代码细节
- **关键**: 输出中必须为每条需求标注各端的关键代码位置（文件:行号）
- **不写文件**，分析结果通过 Agent 返回值传递
- 完成后用 TaskUpdate 标记任务完成
...
向用户输出：`需求三维分析完成，启动需求分析整合...`

启动单个 Agent（普通 Agent tool，无需 team）：

**Agent: 需求分析**
- 读取 `agents/product/requirement-review.md` 作为 prompt 基础
- 注入上下文: 需求文档 + 信息整合结果 + 上面 3 个成员的**返回结果**（内存传递，非文件）
- 输出写入: `{output_dir}/阶段2-需求分析/需求分析.md`（阶段二唯一输出文件）
- 输出核心内容:
  - **逐条需求分析**: 每条需求的清晰度、可行性、代码定位（精确到文件和行号）、技术风险、验收标准
  - **待用户确认事项**: 需求不清晰点、技术风险确认、不可行项的替代方案、范围确认
  - 建议的开发顺序和整体评估

### 3.3 HITL 检查点（必须执行）

**此检查点始终执行**，不受 `hitl.checkpoints` 配置控制，因为用户确认是需求分析的核心环节。

1. 向用户展示需求分析文档中的**「待用户确认事项」**部分
2. 询问：
   ```
   === 需求分析完成 ===

   请查看需求分析文档: {output_dir}/阶段2-需求分析/需求分析.md

   待确认事项:
   [列出所有待确认项，包括需求不清晰点、技术风险、不可行项]

   请逐一确认或补充说明：
   - 对每个待确认项回复确认/补充/调整
   - 输入「全部确认」→ 接受所有项，进入架构设计
   - 输入「修改需求」→ 修改需求文档后重跑本阶段
   ```
3. 收到用户反馈后：
   - 如果用户补充了信息，将补充内容**追加到需求分析文档末尾**（「用户确认与补充」章节）
   - 如果用户要求修改需求，等待修改后重新执行阶段二
   - 确认完成后，进入阶段三
...
- 注入: 需求分析文档（验收标准部分） + API 契约 + 各端变更记录
...
- 注入: 当前阶段名称 + 当前阶段所有产出 + 阶段二需求分析文档中的风险摘要 + 前序阶段的风险更新
