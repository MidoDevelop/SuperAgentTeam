...
**目标**: 基于信息采集的代码上下文，从用户、技术、风险三个视角分析需求，形成结构化需求。

### 3.1 创建任务 & 并行启动 3 个分析成员

**创建任务**（使用 TaskCreate，一次创建 3 个）：
1. `用户视角分析` — 从用户体验角度分析需求
2. `技术可行性评估` — 基于代码扫描结果评估技术可行性
3. `风险分析` — 基于代码上下文进行多维度风险评估
...
- 输出写入: `{output_dir}/阶段2-需求分析/需求评审.md`（阶段二唯一输出文件）
...
- 注入: 当前阶段名称 + 当前阶段所有产出 + 阶段二需求评审中的风险摘要 + 前序阶段的风险更新
